...
logger = setup_project_logger("finetuning_data_generator_exp2")
...
def process_csv_for_finetuning(input_csv_path, all_train_data, all_test_data, all_validate_data):
    ...
    by grouping by the 'selected' column, and appends the formatted data to the
    provided lists. Each record in the output will represent a single title.
    ...
    """
    logger.info(f"Starting data processing for fine-tuning from '{input_csv_path}' (Experiment 2)")
    ...
    grouped_by_selected = df.groupby('selected')
    logger.info(f"Grouped data by {len(grouped_by_selected)} unique 'selected' statuses.")

    for selected_status, group in grouped_by_selected:
        logger.debug(f"Processing group for 'selected' status: '{selected_status}' with {len(group)} entries.")
        
        # Get titles and selected status for the current group
        ...
        if not titles_and_selected:
            logger.warning(f"No valid titles found for 'selected' status '{selected_status}'. Skipping this group.")
            continue

        # Step 3: Create a 60-20-20 train-test-validate split randomly for each group
        ...
        train_data_group = titles_and_selected[:train_end]
        test_data_group = titles_and_selected[train_end:test_end]
        validate_data_group = titles_and_selected[test_end:]

        logger.debug(f"  Split for selected status '{selected_status}': Train={len(train_data_group)}, Test={len(test_data_group)}, Validate={len(validate_data_group)}")

        # Step 4 & 5: Convert each individual (title, selected) pair into the specified JSON format
        # and combine the groups into train, test, and validate lists
        for title, selected in train_data_group:
            formatted_entry = _format_to_json(title, selected)
            if formatted_entry:
                all_train_data.append(formatted_entry)
        
        for title, selected in test_data_group:
            formatted_entry = _format_to_json(title, selected)
            if formatted_entry:
                all_test_data.append(formatted_entry)
        
        for title, selected in validate_data_group:
            formatted_entry = _format_to_json(title, selected)
            if formatted_entry:
...
def generate_finetuning_data_for_directory(titles_and_urls_folder, output_finetuning_dir):
    ...
    for csv_file_name in csv_files_in_dir:
        ...
        logger.info(f"\n--- Processing CSV for Exp2: '{csv_file_name}' ---")
        ...
        logger.info(f"--- Finished processing CSV for Exp2: '{csv_file_name}' ---\n")

    logger.info("All relevant CSV files processed for fine-tuning data generation (Experiment 2).")

    # Step 6: Write the combined data to single files with _exp2 suffix
    output_train_path = os.path.join(output_finetuning_dir, "train_exp2.jsonl")
    output_test_path = os.path.join(output_finetuning_dir, "test_exp2.jsonl")
    output_validate_path = os.path.join(output_finetuning_dir, "validate_exp2.jsonl")
